...
## 4. Provisional Config Bootstrapping

A starter provisional configuration can be generated by the package CLI.
It writes a `provisional_featurization_config.yaml` file populated with runtime anchor placeholders, default path values, and a sample pipeline stage.

This helper is useful for data scientists who want a starting point before finalizing project-specific paths and stages.
...
To replace an existing provisional config, use:
...
Once the starter config exists, update the values and pipeline entries to match your dataset and stage definitions.

## 5. Extension Rule (Important)
